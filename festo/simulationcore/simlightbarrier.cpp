...
allitems(items), position(position), state(regs), bitmask(bitmask), manualInterrupted(false) {

}

void SimLightBarrier::evalTimeStep(unsigned int simTime){
    if(!manualInterrupted) {
...
void SimLightBarrier::setManualInterrupted(bool state) {
    this->manualInterrupted = state;
};
