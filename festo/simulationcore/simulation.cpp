...
#else
Simulation::Simulation(SimItemHandling* itemhandling, SimHCI *hci, SimLBI *lbi, ISimInitCompleteObserver *ich) : feedseparator(&shadow), drive(&shadow), siminitobserver(ich) {
#endif
    ...
    this->hci = hci;
    if (this->hci != nullptr) {
        this->hci->setShadow(&shadow); // Works on shadow because its part of the simulation cycle
    }

    this->lbi = lbi;
    if(this->lbi != nullptr){
        this->lbi->setShadow(&shadow);
        this->lbi->setLbBegin(lBBegin);
        this->lbi->setLbHight(lBHight);
        this->lbi->setLbFeedseparator(lBFeedseparator);
        this->lbi->setLbSlide(lbSlide);
        this->lbi->setLbEnd(lbEnd);
    }
...
void Simulation::doSimulationCycle() {
    ...
    if (displayCounter == 0) {
        LOGGER<<"<SIM> Time: " << simTime / 1000.0 << " s" << endl;
    ...
    if (simreleased) {
        ...
        // update manipulation of lightbarriers
        if (lbi != nullptr) {
            lbi->evalTime(simTime);
        }
        ...
        if (reporthandler.size() > 0) {
            ...
            LOGGER << "<Sim> state:" << report << std::endl;
#endif
        ...
        LOGGER << "<SIM> cycle duration:" << elapsed_microsec.count() << endl;
#endif
    }
};
...
void Simulation::evaluateRealTimeStep() {
    ...
    unsigned int elapsed_milliseconds = (simTick - last_update_tick);

    if (elapsed_milliseconds >= SimulationBase::realTimeSlice) {
        last_update_tick = simTick;
...
std::string Simulation::generateJSONStatusReport() {
    stringstream json;
    json << "{";
    // here we need the ID of the system
    json << "\"id\": \"A\", ";
    json << "\"T\": ";
    json << simTime;
    json << ", \"actors\": ";
...
unsigned long Simulation::getSimTime() const {
    return simTick;
}
