    ...
    if (allitems != nullptr) {
        ...
        switch (action.actionkind) {
            ...
            case SimItemHandlingActionKind::removeatend:
            {
                ...
                if (found != nullptr) {
#ifdef SIM_SHOW_ACTIONS
                    LOGGER << "<SIM> Removed at end" << endl;
#endif
            ...
            {
                LOGGER << "<SIM> Not Implemented Yet (removeatbegin)" << endl;
                break;
            }
            ...
            default:
                LOGGER << "<SIM> Not Implemented Yet (default)" << endl;
                ;
        ...
        if ((0 <= (*it)->x) && (40 >= (*it)->x)) {
...
#ifdef SIM_SHOW_ROI
                LOGGER << "<SIM> Item " << (*it)->ID << "/" << kind.str() << " entered entry-region" << endl;
#endif
            }
        } else {
            // in slide
            if (90 <= (*it)->y) {
                if ((*it)->roi != RoI::discarded) {
                    ...
                    (*it)->typeToShortTypeName(kind);
#ifdef SIM_SHOW_ROI
                    LOGGER << "<SIM> Item " << (*it)->ID << "/" << kind.str() << " entered discard-region" << endl;
#endif
               }
            } else {
                // end of belt
                if ((655 - 20 <= (*it)->x) && (655 + 20 >= (*it)->x)) {
                    if ((*it)->roi != RoI::exit) {
                        ...
                        (*it)->typeToShortTypeName(kind);
#ifdef SIM_SHOW_ROI
                        LOGGER << "<SIM> Item " << (*it)->ID << "/" << kind.str() << " entered exit-region" << endl;
#endif
                    }
                } else {
                    (*it)->roi = RoI::none;
