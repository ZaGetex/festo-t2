...
#include "simhci.h"
#include "simlbi.h" //Added by Tobias Jaehnichen, 14.04.2021
...
private:
    ...
    unsigned long simTick = 0; // [ms]
    unsigned long last_update_tick = 0; // [ms]
...
private:
    ...
    std::vector<std::shared_ptr < SimItem>> conveyoritems;
    std::vector<std::shared_ptr < SimItem>> slideitems;
    std::vector< ISimulationCycleEndHandler*> cylceendhandler;
    std::vector< ISimulationReportHandler*> reporthandler;
    ISimInitCompleteObserver *siminitobserver;
    std::mutex buffermutex;
    unsigned int instancenumber;
    static unsigned int instancecounter;
public:
    Simulation(SimItemHandling* itemhandling = nullptr, SimHCI *hci = nullptr, SimLBI *lbi = nullptr, ISimInitCompleteObserver *ich = nullptr);
    ...
    unsigned long getSimTime() const;
private:
    //void updateSimTime();
